...
class PriceVisualizer:
    ...
    def plot_10_year_trend(self, output_dir: str = "output", show: bool = False) -> str:
        ...
        ax.scatter(ath_row["date"], ath_row["price"], color='#ef4444', s=120, zorder=5, marker='^', label=f'All-Time High ({curr_symbol}{ath_row["price"]:,.2f})')
        ...
        ax.set_title(f"PricePulse 10-Year Price Trend: {self.product.title}", fontsize=16, fontweight='bold', color='#ffffff', pad=20, bbox=dict(boxstyle="round,pad=0.4", fc="#1e293b", ec="#3b82f6", lw=1.5))
        ...
        legend = ax.legend(loc='upper right', facecolor='#0f172a', edgecolor='#3b82f6', labelcolor='#ffffff', fontsize=10, framealpha=0.95)
