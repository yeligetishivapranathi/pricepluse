...
def render_sidebar():
    ...
    if saved_prods:
        ...
        choice = st.sidebar.selectbox("Select Stored Product:", options)
        if choice != "-- New Search --":
            idx = options.index(choice) - 1
            selected_saved = saved_prods[idx]
...
def main():
    ...
    saved_product = render_sidebar()

    # Search Bar
    st.markdown("### 🔍 Track New Product")
    col1, col2 = st.columns([4, 1])
    with col1:
        default_query = saved_product.title if saved_product else "Sony WH-1000XM5"
        query_input = st.text_input("Enter any Product Name:", value=default_query, placeholder="e.g. iPhone 15 Pro, PlayStation 5, MacBook Air M2, Nike Air Max")
    with col2:
        st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
        search_btn = st.button("🚀 Analyze Price", use_container_width=True)

    if search_btn or saved_product or "active_product" in st.session_state:
        query = query_input.strip()
        
        if search_btn or ("active_product" not in st.session_state or st.session_state.get("last_query") != query):
            if saved_product and not search_btn:
        ...
        with col_right:
            ...
            if "BUY" in rating_text or "DEAL" in rating_text:
                badge_class = "badge-buy"
            elif "OVERPRICED" in rating_text:
